  ...
  try {
    if (transactionType === "Transfer") {
      ...
      const findCard = await Card.findOne({
        userId: findfromUser.customerName,
      });
      if (!findCard) {
        return res.status(404).json({
  ...
  try {
    ...
    const pastData = userlist.map((transaction) => {
      ...
      const transactionAmount =
        isSender && transaction.transactionType !== "Withdraw"
...
export const postPhoneNumber = async (req, res) => {
  ...
  try {
    ...
    if (findPhoneNumber) {
      return res.status(200).json({
        message: "Fetch PhoneNumber Successfully.",
